...
| AI開発ワークフロー | TAKT、Codex CLI | `mise.toml`で固定 | 計画、実装、レビュー、修正のワークフロー化 | ChatGPTプランで認証したローカルCLIを使用し、APIキーは使用しない |
...
- Go、golangci-lint、Node.js、TAKT、Codex CLI、パッケージ化ツールは`mise.toml`で固定する。
